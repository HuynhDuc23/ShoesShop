...
public class OrderServiceImpl implements OrderService {
    ...
    @Override
//    @KafkaListener(topics = "create-order", groupId = "consumer-order")
    @Transactional
    public Order createOrder(CreateOrderRequest createOrderRequest, Long id) {
        // get user from springcontextholder
        //User userFromAuthentication = ((CustomUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getUser();
        // long userId
        ...
        //Kiểm tra size có sẵn , và kiểm tra số lượng hàng tồn kho
        ...
        // Giảm số lượng tồn kho check truong hop khong con so luong
        productSize.setQuantity(productSize.getQuantity() <= 0 ? 0 : productSize.getQuantity() - 1);
...
//        this.mailService.sendMailSimple("ductrantad23@gmail.com", "Thông báo đơn hàng", "Đơn hàng của bạn đã được tạo thành công");
        orderRepository.save(order);
        return order;

    }
    @Override
    @KafkaListener(topics = "create-order", groupId = "consumer-order", containerFactory = "kafkaListenerContainerFactory")
    @Transactional
    public Order createOrder(CreateOrderRequest createOrderRequest) {
        try {
            ...
            String email = this.userRepository.findById(createOrderRequest.getUserId()).get().getEmail();
            if(email != null){
                this.mailService.sendMailSimple(email, "Thông báo đơn hàng", "Đơn hàng của bạn đã được tạo thành công");
            }
            // lấy imail của user tại đây
            return order;
        } catch (ListenerExecutionFailedException | MessagingException ex) {
            // Log và xử lý khi Kafka listener không thể thực thi message
    ...
    public void userCancelOrder(long id, long userId) {
        ...
        //Cộng lại một sản phẩm đã bị trừ
        var data =  this.productSizeRepository.findByProductId(order.getProduct().getId());
        for (ProductSize productSize : data) {
            if (productSize.getSize() == order.getSize()) {
                productSize.setQuantity(productSize.getQuantity() + 1);
                productSizeRepository.save(productSize);
            }
        }
    ...
    public Order findById(Long orderId) {
        return this.orderRepository.findById(orderId).orElseThrow(() -> new NotFoundException("Đơn hàng không tồn tại"));
    }

    @Override
    public boolean updateOrderStatus(String orderId, String status) {
        try {
            Order order = orderRepository.findById(Long.parseLong(orderId)).orElseThrow(() -> new NotFoundException("Đơn hàng không tồn tại"));
